...
- **AI agent 연동 (`services/agent_client.py`)**: 아직 mock. 팀원의 AI agent가 별도 서비스로
  ...
  방식은 미정. (한 번 실제 스펙으로 보이는 문서를 보고 연동을 시도했었으나, 그 문서는
  강사 측이 제공한 벤치마킹용 가이드 코드였고 teammate의 실제 스펙이 아니었음이 확인되어
  다시 mock으로 되돌림 — 2026-07-09.) 스펙이 정해지면 `ask_ai_agent()` 함수 내부만 실제
  HTTP 호출로 교체하면 되고, 라우트/스키마 등 나머지 코드는 그대로 유지된다.
